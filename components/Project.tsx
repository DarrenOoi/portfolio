...
const Project = ({
  ...
  return (
    <div className='flex flex-col px-4 md:px-44 pb-2'>
      <h1 className='text-5xl mb-2 font-bold text-black'>{title}</h1>
      <p className='text-xl text-[#506d8a] mb-2'>{subtitle}</p>
      {icons && (
          ...
          {icons?.map((icon, index) => {
            ...
            return (
              <a href={tech.link} key={index} title={tech.name} target='_blank'>
                <i
                  key={index}
                  className={`devicon-${icon} opacity-75 text-3xl mr-2 hover:text-[#63C132]`}
                ></i>
              </a>
            );
          })}
      ...
      <p className='text-m mt-2 mb-4 text-black'>{description}</p>
      ...
      <div className='w-5/6 h-0.5 bg-[#506d8a] mx-auto mb-4' />
    </div>
  );
};
